...
import { retryAgentRun } from '@/lib/server/agent-runs';
...
export async function POST(
  ...
  try {
    ...
    const run = await retryAgentRun(id);
    return successResponse(run, 'retried');
  } catch (err) {
    ...
    console.error('[agent-runs retry POST] unexpected error:', err);
    return NextResponse.json(
      {
        code: 50000,
