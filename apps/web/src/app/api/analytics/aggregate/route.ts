...
import { getAnalyticsAggregate } from '@acs/analytics-center';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await getAnalyticsAggregate(days, {
      platform,
      contentType,
      accountId,
    });
    ...
    console.error('[analytics aggregate] unexpected error:', err);
    return NextResponse.json(
      {
        code: 50000,
